...
/**
 * This is the controller for 'modifyProduct.fxml.' It executes methods when an
 * FXML element in the the Modify Product page is pressed/interacted with.
 *
...
public class ModifyProductController implements Initializable {
    ...
    // This is the product we are modifying
    ...
    // Our title. Later we will add a product ID number to it.
    @FXML
    private Text title;
    ...
    // Initializes the values and columns for the Modify Product page.
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        // Initialize the title. The product ID number will be added in another method.
        title.setText("Modify Product: ");
        // Sets the ID fields to be uneditable.
    ...
    // Method sets the title to include the ID number of the product.
    public void setTitle(int id) {
        title.setText(title.getText() + id);
    }

    // Method sets the inventory we are using.
    public void setInventory(Inventory passedInv) {
        inventory = passedInv;

        // Sets the top table view to show the parts not in the product
        searchView.setItems(otherParts);

    }

    // Method sets the product we are editing
    public void setProduct(Product prod) {
        // Assign the product we are passed from main menu
        thisProduct = prod;

        // Set the fields to the values of said product
        idField.setText(String.valueOf(prod.getId()));
        nameField.setText(prod.getName());
        stockField.setText(String.valueOf(prod.getStock()));
        ...
        for (Part part : thisProduct.getAllAssociatedParts()) {
            partsToAdd.add(part);
        ...
        // Adds ID number to the title
        setTitle(prod.getId());

        // Checks the inventory for what parts are not in the product
        for (Part invPart : inventory.getAllParts()) {
            if (!thisProduct.getAllAssociatedParts().contains(invPart)) {
    ...
    public void deleteBtnPressed(ActionEvent e) {
        ...
        // Remove this part from our product and add it to the other table.
    ...
    // The goal is to create a new product with our changes and replace the old
    // one.
    public void saveBtnPressed(ActionEvent e) {
        ...
        try {
            ...
            for (Part toDelete : otherParts) {
                if (thisProduct.getAllAssociatedParts().contains(toDelete)) {
            ...
            // Method updates the inventory by deleting the old product and adding
            // the new one. This method will not add extra instances of parts
            inventory.updateProduct(thisProduct.getId(), newProduct);
